...
def test_conversation_messages_relationship(app):
    with app.app_context():
        ...
        conversation = Conversation(
            user_id=user.id, telegram_chat_id="chat-123", state="new", context="{}"
        )
        db.session.add(conversation)
        db.session.commit()

        first_message = Message(
            conversation_id=conversation.id, sender="user", text="Hello"
        )
        second_message = Message(
            conversation_id=conversation.id, sender="bot", text="Hi there"
        )
