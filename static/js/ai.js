...
function bindGenerateRepliesButton(message) {
  ...
  generateBtn.addEventListener("click", () => {
    ...
    if (!emailBody.trim()) {
      if (wrap)
        wrap.innerHTML = `<span style="color:#888;">No email content.</span>`;
      return;
    }
    ...
    fetch("/nlp/suggest", {
      ...
      .catch(() => {
        const list = document.getElementById("suggestions-list");
        if (list)
          list.innerHTML = `<span style="color:#888;">Failed to generate replies.</span>`;
      });
  });
}
...
async function handleAiWrite() {
  ...
  try {
    const res = await fetch("/nlp/ai-draft", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
